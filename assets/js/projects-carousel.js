...
(function () {
  ...
  /* ── "一格一格滚轮"的判据要整轮一致（第三百三十批，主人「修复一下bug，左右连续滑动时，
        动画有时会很奇怪。还有有时有bug，不知道为什么可以一次滑动多张卡片」）────────────
     先量后改（`scratch/pj-damp-2026-09-27/probe-notchy.js`：真机录制原时间轴 + `__ev`/`__land`
     两本仪表；`jank` 档在主线程每 rAF 里占 30ms，模拟"页面在跑三个画布"时的处理间隔）：

       场景                          段数  单帧最大位移  落位 |k|>1   Δslot
       真机连续两划（不卡）            2       24px        0 次        2     ✓
       真机连续两划 + 每帧占 30ms      3      286px        1 次        6     ✗
       左划→右划（不卡）               4       26px        0 次        0     ✓
       左划→右划 + 每帧占 30ms         6     1728px        1 次       20     ✗

     病根不在跟手，在**类别判据**：老规则是"任意一条事件满足（位移 ≥60px 且**处理间隔** ≥40ms）
     就判成滚轮，判了就锁到本轮结束"。主线程一卡，处理间隔会被拉到 62~63ms（实测），
     这时只要来一条 ≥60px 的位移，触控板就被当成"一格一格滚轮"，后果三条：
       ① 落位的 k **不再夹在 ±1**（滚轮那条按距离走）⇒ **一次落位滑走好几张**（实测 k = −4 / −24）；
       ② 跟手从软跟手换成微跟手（40px 封顶）⇒ **动画突然变样**（"左右连续滑动时动画很奇怪"）；
       ③ 账本按距离走 ⇒ 锚点能被推到十几档之外（实测 锚点 −8827 = 第 20 档、单帧 1728px 直飞）。
     ⚠️ 真机触控板的**事件间隔上限只有 33~39ms**、就贴在 40ms 门槛下面（12 段录制离线扫），
        所以这条判据本来就是"靠卡顿才误判"的 —— 别把门槛调低、也别改成"位移大小"单独判
        （`swipe-left-fast-trackpad` 里有 **50 条 ≥60px** 的位移，只看位移必误判）。
     改法（`sawFine`，**单调撤销 + 跨小节保留**）：只要**这一次滚动会话**里出现过一条"细粒度"事件
     （间隔 <40ms 或位移 <60px），就再也不判滚轮 —— 而这个"不判"要**跨小节保留**（只在停手
     ≥W_CARRY_KEEP 1s 时才清）。为什么必须跨小节：触控板一次手势里天然有 120ms 以上的小停顿
     （每一停就是一次落位 = 一个新"小节"），若按小节清，每一节的**第一条**事件都可能"像一格一格"
     （首条的间隔天然很大）⇒ 判据在每节开头翻一次身，跟手目标在"微跟手 40px"与"整段跟手"之间
     来回换 = 主人说的"左右连续滑动时动画有时很奇怪"（实测 jankAlt：16 次翻转 + 单帧 699px）。
     跨小节之后：触控板手势里一条细粒度事件就把它钉死；真滚轮（60ms / 250ms 规整一格一格）
     一条细粒度事件都不会有 ⇒ 照旧按距离走。
     ⚠️ **别再加"连续 ≥N 条才认"**（试过、量出来是坏的）：慢滚（250ms/格）每条都自成一段
       （>W_IDLE 120ms ⇒ 每格落位一次），前 N 条按触控板那条夹在 ±1、第 N 条之后才按距离走
       ⇒ 两套账混用，`diag-big` 的慢滚列从 1/1/2/3/4 变成 **1/1/3/4/5**（8 格走了 3 张）。
       单调撤销没有这个问题：第一格就认，从头到尾一套账 ✅ 实测 1/1/2/3/4 = 330 批前一致。
     ⚠️ 口径变化（写进探针）：`diag-big` 的"连续推 20ms/事件、120px"那一列**从"按距离"变成
       "一次一张"** —— 20ms 间隔的连续流按 324 批的定义就是"细粒度输入"（Magic Mouse 同族），
       本来就该走触控板那条（主人 321 批的"一次接触一张"）。 */
  ...
  /* 第三百三十批反证档：`?cnotch=1` = **老判据**（任意一条满足就认、认了锁到本轮结束）——
     只给套件 C12 用（复现"卡顿把触控板判成滚轮 ⇒ 一次落位多张"）。 */
  var NOTCH_OLD = /[?&]cnotch=1/.test(location.search);
  var wheelNotchy = false;  /* 本轮是不是"一格一格"的滚轮输入（见文件头 324/330 批） */
  var sawFine = false;      /* 这一次滚动会话里出现过"细粒度"事件 ⇒ 再也不判滚轮
                               （单调撤销 + **跨小节保留**，只在停手 ≥1s 时清，见文件头 330 批） */
  ...
  function voidWheelRound() {
    ...
    sawFine = false;                    /* 第三百三十批：本轮作废 ⇒ 类别重新取 */
  }
  function wheelEnd() {
    wheelIdleTimer = null;
    if (!wheeling) return;
  ...
  root.addEventListener('wheel', function (e) {
    ...
    /* 第三百二十四批：是不是"一格一格"的滚轮输入（灯箱实测特征，见 W_NOTCH_MIN 那段）。
       ...
       第三百三十批：**判据加一条"单调撤销"**（见文件头 330 批）—— 老写法是"任意一条满足就认、
       认了就锁到本轮结束"，主线程一卡（处理间隔被拉到 ≥40ms）就会把触控板当成滚轮 ⇒
       一次落位滑走好几张 + 跟手换类 + 锚点飞走。现在：本轮只要出现过一条细粒度事件，
       本轮永不判滚轮（只许撤销、不许翻转）。 */
    if (tpNewAction || PAGE_PER_GESTURE && wheelGap >= W_CARRY_KEEP) wheelNotchy = false;
    /* ⚠️ `sawFine` **只在停手 ≥1s 时清**（跨小节保留）—— 按小节清会让每一节的第一条事件
       （间隔天然很大）都可能判成滚轮，跟手目标每节翻一次身（见文件头 330 批实测）。 */
    if (PAGE_PER_GESTURE && wheelGap >= W_CARRY_KEEP) sawFine = false;
    if (tpNewAction) burstN = 0;      /* 新出手 ⇒ 这一串从头数（第三百二十八批） */
    var notchHit = !tpRes.momentum && Math.abs(d) >= W_NOTCH_MIN && wheelGap >= W_NOTCH_GAP;
    if (NOTCH_OLD) {                                   /* 反证档：老判据（见 NOTCH_OLD 那段） */
      if (notchHit) wheelNotchy = true;
    } else {
      if (notchHit && !sawFine) wheelNotchy = true;
      else { sawFine = true; wheelNotchy = false; }    /* 细粒度事件 ⇒ 本轮永不判滚轮（单调） */
    }
    ...
    if (!wheeling) {
      ...
      wheelSegSpent = false;
      burstN = 0;
      /* 第三百三十批：`sawFine` 在这里**故意不清** —— 它是"这一次滚动会话"级的（见判据那段） */
      wHist.length = 0;
      segs++;
    }
    wheelAcc += d;
  ...
  if (CAR_DEBUG) {
    window.__car = function () {
      ...
      return {
        ...
        notchyOld: NOTCH_OLD,                      /* 第三百三十批反证档：老判据在档 */
